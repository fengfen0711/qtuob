			...
			<div class="Group">
				...
				<div class="twoCon">
					...
					<p class="inputGrop clearFloat" v-for="(item,index) in tabs" yn="0" ref="father" :code="item.quesNo">
						<label class="inputLabel">{{item.quesCont}}</label>
...
<script>
	...
	export default{
		...
		created(){
			...
			if(a!=undefined){
				this.a=JSON.parse(localStorage.dataList)
			}else{
				var _this=this
				Dictionaries().then((res)=>{
					 _this.a=res
				...
				this.$http.post(this.$store.state.link5 + '/trd/order/v1/queryorder', data)
					.then(res => {
						if(res.data.code == "SYS_S_000") {
							console.log("11=="+JSON.stringify(res.data.output))
							if(res.data.output.applNotResp.length!=0){
								this.backview=res.data.output.applNotResp
								this.flag=true
							}
...
//							this.$router.push('/insuranceadvice?prodCode=' + this.$route.query.prodCode + "&orderNo=" + this.$route.query.orderNo + "&cmpCode=" + this.$route.query.cmpCode + "&userId=" + this.$route.query.userId + "&prodNo=" + this.$route.query.prodNo + "&token=" + this.$route.query.token)
							this.$router.push('/healthTold1?prodCode=' + this.$route.query.prodCode + "&orderNo=" + this.$route.query.orderNo + "&cmpCode=" + this.$route.query.cmpCode + "&prodNo=" + this.$route.query.prodNo )
						} else {
