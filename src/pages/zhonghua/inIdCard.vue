<template>
	<div class="one" style=" -webkit-tap-highlight-color: transparent; -webkit-focus-ring-color: transparent;">
		<div class="twoBox">
			...
			<div class="idBox">
				<!--<img src="/static/img/zhonghua/id_z.png" class="idImg" />-->
			...
			<div class="idBox">
				...
				<!--<img src="/static/img/zhonghua/id_f.png" class="idImg" />-->
						...
						if(res.data.code == "SYS_S_000"){
							if(res.data.output.responseMsg=="交易成功"){
								...
								if(this.flage==true&&this.flage1==true){
									this.$router.push("insured?address="+this.address+"&birthday="+this.birthday+"&cardNo="+this.cardNo+"&name="+this.name+"&nation="+this.nation+"&sex="+this.sex+"&issuingDate="+this.issuingDate+"&expiryDates="+this.expiryDates+"&prodCode="+this.$route.query.prodCode+'&prodNo='+this.$route.query.prodNo+'&orderNo='+this.$route.query.orderNo+'&cmpCode='+this.$route.query.cmpCode+"&insuredcardno="+this.code+"&insuredsite="+this.code1)
								}
							}else {
								Toast("ocr识别失败，请重新上传");
							}
			...
			ocrother(code){
				...
				this.$http.post(this.$store.state.link5 + "/doc/v1/ocr", dataCode).then(res => {
						...
						if(res.data.code == "SYS_S_000"){
							...
							if(this.flage==true&&this.flage1==true){
								this.$router.push("insured?address="+this.address+"&birthday="+this.birthday+"&cardNo="+this.cardNo+"&name="+this.name+"&nation="+this.nation+"&sex="+this.sex+"&issuingDate="+this.issuingDate+"&expiryDates="+this.expiryDates+"&prodCode="+this.$route.query.prodCode+'&prodNo='+this.$route.query.prodNo+'&orderNo='+this.$route.query.orderNo+'&cmpCode='+this.$route.query.cmpCode+"&status="+"2"+"&insuredcardno="+this.code+"&insuredsite="+this.code1)
							}
							}else{
								Toast("ocr识别失败，请重新上传");
							}
