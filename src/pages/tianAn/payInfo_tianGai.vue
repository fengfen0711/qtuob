	...
	<div class="one" :class="{opa0:!hui}" style=" -webkit-tap-highlight-color: transparent; -webkit-focus-ring-color: transparent;">
		...
		<div id="blur_all">
			...
			<p v-if="hui" class="btnBox clearFloat" :class="{btnBox1:btnBoxShow}">
				...
				<span class="btn" @click="handleClickNext">确认支付</span>
			</p>
		</div>
	</div>
</template>
					...
					.then(res => {
						...
						if(dataCode == "SYS_S_000") {
							...
							if(this.allData.paymentResp.accName != undefined) {
								...
								this.bankNumberElse = this.allData.paymentResp.accNo.replace(/[\s]/g, '').replace(/(\d{4})(?=\d)/g, "$1 ")
							}
...
//				    console.log(res.data);
			...
			handleClickNext() {
				if(this.sBoxShow) {
					...
					if(this.bankNumber == this.bankNumberElse) {
						this.bankSave(1)
					}else{
						this.bankSave()
					}
			...
			bankSave(a){
				var paymentReqData = {
					...
					"bankName": this.bankCodeName, //银行名称
...
//					Indicator.close();
					...
					if(res.data.code == "SYS_S_000") {
						if (a==1) {
							...
							MessageBox.confirm('',{
							  	title: '温馨提示',
							  	message: "银行卡信息修改，需重新上传银行卡照片并确认签字",
							  	confirmButtonText: '确定',
							  	showCancelButton: false
							}).then(action => {
								if (action == 'confirm') {
									this.$router.push('/bankCardUpload?prodCode=' + this.$route.query.prodCode + "&orderNo=" + this.$route.query.orderNo + "&cmpCode=" + this.$route.query.cmpCode + "&userId=" + this.$route.query.userId + "&prodNo=" + this.$route.query.prodNo + "&token=" + this.$route.query.token)
								}
							})
						}
					};
