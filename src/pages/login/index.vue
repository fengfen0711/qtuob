		...
		methods: {
			...
			handleClickLogin() {
				if(this.phoneNum == "") {
					...
					this.$http.post(this.$store.state.link + '/sso/dologin', data).then(response => {
						...
						if(response.data.code == "SYS_S_000") {
							...
							//							var agentCode = "10000015";
							//							var agentName = "高薇";
							//							var deptCode = "47";
							//							var agentDeptCode = "8001";
							//							var loginData = { //登陆之后的代理人信息
							//								"agentCode": agentCode, //代理人代码 
							//								"agentName": agentName, //代理人名称 
							//								"deptCode": deptCode, //机构代码   
							//								"agentDeptCode": agentDeptCode, //代理机构
							//							}
							this.$store.dispatch("changeToken", response.data.output.token)
							this.$store.dispatch("changeUserId", response.data.output.userId)
							//							this.$store.dispatch("changeLoginData", {})
							//							this.$store.dispatch("changeLoginData", loginData)
							...
							//							console.log("store1==" + JSON.stringify(response.data.output.brokerInfo))
