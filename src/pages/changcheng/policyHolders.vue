			...
			<div class="twoCon">
				...
				<div class="policyHolder">
					<p class="inputGrop clearFloat">
						...
						<select v-model="nexusType" class="inputText inputWidth left" @change="nex">
					...
					<p class="inputGrop clearFloat">
						...
						<span v-model="gender" class="inputText inputSpan left" @click="genderClick">
							<span class="sex sexM" >
					...
					<p class="inputGrop clearFloat">
						...
						<select class="left inputText inputWidth" v-model="eduType">
							<option value="q">请选择</option>
					...
					<div class="inputGrop1 clearFloat">
						<p class="inputP">
							<label class="inputLabel1 left"><label class="star">*</label>常住地址</label>
					...
					<!--<p class="inputGrop clearFloat">
						<label class="inputLabel left"><label class="star">*</label>工作/职务内容</label>
						<input type="text" v-model="wordPost" class="inputText inputWidth left" placeholder="请输入工作/职务内容" />
					</p>-->
					<!--<p class="inputGrop clearFloat">
						<label class="inputLabel left"><label class="star">*</label>单位/学校地址</label>
						<input type="text" v-model="workAssress"  class="inputText inputWidth left" placeholder="请输入单位/学校地址" />
					</p>-->
					<div class="inputGrop1 clearFloat">
						<p class="inputP">
					...
					<div class="insured" v-show="insuerd">
						<p class="inputGrop clearFloat">
...
//					"opt": "MAIN,APPL",
				...
				this.$http.post('http://192.168.171.12:9009' + '/order/v1/queryorder', data).then(res => {
					...
					if(dataCode == "SYS_S_000") {
						...
						this.chengshi(); //城市
						this.chengshi1(); //城市
						if(res.data.output.applntResp.relationToInsured == "A"){
							this.insuerd = true
						...
						if(this.allData.applntResp.relationToInsured != undefined) {
							this.nexusType = res.data.output.applntResp.relationToInsured //关系
						} else {
							this.nexusType = "q";
						}
						this.appName = this.$route.query.appName 
						if(this.allData.applntResp.certfType != undefined) {
							this.cardType = res.data.output.applntResp.certfType //证件类型
						} else {
							this.cardType = "q";
						}
						...
						if(this.allData.applntResp.maritalStatus != undefined) {
							this.marType =  res.data.output.applntResp.maritalStatus;//婚姻状况
						} else {
							this.marType = "q";
						}
						if(this.allData.applntResp.appntDegree != undefined) {
							this.eduType = res.data.output.applntResp.appntDegree //教育程度
						} else {
							this.eduType = "q";
						}
						this.phone = res.data.output.applntResp.mobile //  手机号码
						this.email = res.data.output.applntResp.email // 邮编
						this.add = res.data.output.applntResp.fullAddr //  乡镇地址
						this.address = res.data.output.applntResp.address
						this.work_address =  res.data.output.applntResp.workCompany// 工作单位
						this.partTime =  res.data.output.applntResp.appntPlurality
					} else {
						this.allType();
						this.chengshi(); //城市
						this.chengshi1(); //城市
...
//				
			...
			chengshi1() {
				...
				this.$http.post(this.$store.state.link + '/dic/findChinaByOrgCode', data)
					.then(res => {
						var dataCode = res.data.code;
						console.log(this.allData.applntResp)
						if(dataCode == "SYS_S_000") {
							this.provinceList1 = res.data.output;
							if(this.allData.applntResp.workAssress!= undefined) {
								for(var i = 0; i < this.provinceList1.length; i++) {
									if(this.provinceList1[i].cnName == this.allData.applntResp.workAssress.split(",")[1]) {
							...
							if(index == "select") {
								...
								if(this.allData.applntResp.workAssress!= undefined){
									for(var i = 0; i < this.cityList1.length; i++) {
										if(this.cityList1[i].cnName == this.allData.applntResp.workAssress.split(",")[3]) {
							...
							if(index == "select") {
								...
								if(this.allData.applntResp.workAssress!= undefined) {
									for(var i = 0; i < this.countyList1.length; i++) {
										if(this.countyList1[i].cnName == this.allData.applntResp.workAssress.split(",")[5]) {
			...
			getNextClick(){
				...
				if(this.IDnum == ''){
					...
					if(this.IDnum.length < 6) {
						Toast('投保人证件为军官证或警官证时，证件号码位数必须必须不少于6个字符')
						return
					}
				}else if(this.cardType == "F" ) {
					if(this.IDnum.length < 3) {
						Toast('投保人证件为护照时，证件号码位数必须不少于3个字符')
						return
					}
				}else if(this.cardType == "W" ) {
					if(this.IDnum.length < 8) {
						Toast('投保人证件为港澳台回乡证时，证件号码位数必须不少于8个字符')
						return
					}
				}else if(this.cardType == "D" || this.cardType == "C") {
					if(this.IDnum.length <3) {
						Toast('投保人证件为出生证或户口本号码时，证件号码位数必须不少于3个字符')
						return
					}
				}
				if(this.reHighs == "q"){
...
//				}
				...
				if(this.phone == "" || this.phone == undefined) {
					Toast('请输入您的手机号');
					return;
				}else if(this.phone != "" && this.phone != undefined && !regPh.test(this.phone)) {
					Toast('手机号码格式不正确');
					return;
				}
...
//				if(this.wordPost.length <= 0){
//					Toast("工作/职务内容不得为空")
//					return
//				}
//				
//				if(this.workAssress.length <= 0){
//					Toast("单位/学校地址不得为空")
//					return
//				}
...
//							"occNo": this.occNo,//职业代码
						...
						var msg1 = {
							...
							"city": this.cityType[0], //地址市
							"county": this.countyType[0], //地址区县 
							"fullAddr": this.add, //整乡街道,	
							"workCompany": this.work_address, //工作单位 /学校,
							"address":this.address,
//							"position":this.wordPost,//工作职务/学校内容,
//							"occName": this.occName, //职业
//							"occNo": this.occNo,//职业代码
							"appntPlurality":this.partTime,//兼职
							"workAssress":this.provinceType1+','+this.cityType1+','+this.countyType1,
							"custIdentity": this.zoneType, //居民税收类型
						}
						var msg2 = {
									"agentCode": "test",
						...
						this.$http.post('http://192.168.171.12:9009' + '/order/v1/saveorder', data)
							.then(res => {
								Indicator.close();
								console.log(this.provinceType1[0]+','+this.cityType1[0]+','+this.countyType1[0])
