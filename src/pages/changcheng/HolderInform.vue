	...
	<div class="insuranceadvice_all" :class="{opa0:!hui}" ref='bg'>
		<div class="insuracce_bg">
			<div v-if="hui" class="insuranceadvice_div">
				<p class="insuranceadvice_thing">健康告知事项</p>
			</div>
			<div v-show="item.showFlag==1" :showOrder="item.showOrder" ref="father" :flag="item.showFlag" class="father" v-for="(item,index) in tabs" yn="0" :status="item.status" style="clear: both;" :code="item.quesNo" :key="item.key">
				<div class="insuranceadvice_line" style="clear: both;">
					...
					<div class="insuranceadvice_quest" v-show="item.quesOptList!=undefined">
						<div class="insurance_yes" @click="insurance_yes($event,index)">是</div>
						<div class="insurance_no" @click="insurance_no($event,index)">否</div>
					</div>
				</div>
			...
			this.$http.post(this.$store.state.link + '/prd/prod/heallist', data)
				.then(res => {
					if(res.data.code == "SYS_S_000") {
						...
						console.log('%c heallist','color:#00CD00',JSON.parse(JSON.stringify(this.tabs)))
...
//							this.table.push(this.backData[i].noticeAnswer2)
			...
			postData(mery) {
				...
				Indicator.open()
				this.$http.post(this.$store.state.link6 + '/trd/order/v1/saveorder', data).then(response => {
					Indicator.close()
